...
- `git_worktree_manager` supports read-only `status`, `push_plan`, `worktrees`, and `diff_preview` previews from real Git commands. `diff_preview` returns working-tree, staged, and available branch-range sections with structured file metadata for the Desktop Git panel. `commit` and `push` may execute real Git only with Core-supplied, Core-verified, `kind=git` approved `approval_id` plus explicit `confirm_commit` / `confirm_push`; path commits must stay inside the worktree, and push must block on dirty/behind/detached states. Merge, rebase, branch creation, checkout, and worktree creation remain blocked after approval until implemented deliberately.
