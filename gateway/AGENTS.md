...
- `git_worktree_manager` retains read and index actions as compatibility adapters: reads and `stage`/`unstage` forward to approval-gated TinadecTools tools, so Gateway must not execute Git locally for them. `git_stage` / `git_unstage` accept full paths or a validated text patch and require a Core-verified `kind=git` approval. Commit and push remain on the manager path with Core approval and explicit confirmation; merge, rebase, branch creation, checkout, and worktree creation remain blocked after approval until implemented deliberately.
