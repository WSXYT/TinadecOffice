...
**Last Updated By:** Codex (Git index tools)
**Last Verified Commit:** 2a96248
**Branch:** main
...
- `executor_git_manager` is the execution-layer Git Manager Subagent. It may explain branch/status/diff/worktree/push readiness; `git_stage` and `git_unstage` update the index through approved TinadecTools calls, while commit and push remain approval-gated through `git_worktree_manager`.
